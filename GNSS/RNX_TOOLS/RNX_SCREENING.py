...
# Filtrar os avisos que deseja ignorar
warnings.filterwarnings("ignore", message="Ignoring XDG_SESSION_TYPE=wayland on Gnome.")
...
MW_combination = melbourne_wubbena_combination(f1, f2, L1_array, L2_array, P1_array, P2_array)
MW_combination2 = melbourne_wubbena_combination(f1, f5, L1_array, L5_array, P1_array, P5_array)


IFL_combination = iono_free_phase_combination(f1, f2, L1_array, L2_array)
IFP_combination = iono_free_range_combination(f1, f2, P1_array, P2_array)
...
for i, arc in enumerate(arcos):
    ...
    polynomial_fit = fit_polynomial(x_values, arc_values, 3)
...
for i, (arc, ax) in enumerate(zip(arcos_validos, axes.flatten()), start=1):
    ...
    y_rescaled = rescale_data(arc_values)
...
#
    ...
    for i, (mini_start, mini_end) in enumerate(mini_arcos_mantidos):
        ...
        try:
            mini_fit = fit_polynomial(mini_time, mini_residuals, 3)
            ...
            mini_high_residuals_mask = new_mini_resilevarduals > 0.0002  # Máscara para resíduos altos
