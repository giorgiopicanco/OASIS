...
for c in constellations:
    ...
    for sat in sorted_common_elements_list:
        ...
        # Formatar e imprimir no formato hh:mm
        formatted_time = time.strftime("%H:%M:%S", local_time)
        # print(formatted_time)



        pd.set_option('display.max_rows', None)  # Define para None para mostrar todas as linhas
        # print(df_filtrado.head(10))  # Mostra as primeiras 10 linhas
        ...
        # Definir a taxa de interpolação em segundos (exemplo: 15 segundos)
        rate = freq


        # Listas para armazenar os dados originais e interpolados
        all_data = []
        all_time = []
        all_sat = []
        all_longitude = []
        ...
        # Variáveis para armazenar os últimos dados conhecidos de cada satélite
        ...
        for _, row in df_filtrado.iterrows():
            ...
            # Converte a data e a hora atuais para um objeto datetime para calcular intervalos
            current_time = datetime.strptime(f"{date} {time}", "%d-%m-%Y %H:%M:%S")

            # Se já temos dados anteriores para o satélite, verificamos a necessidade de interpolação
            if sat in last_data_by_satellite:
                last_known_time = last_data_by_satellite[sat]['date_time']

                # Se a diferença entre o último tempo conhecido e o tempo atual for maior que o rate, interpolamos
                while last_known_time + timedelta(seconds=rate) < current_time:
                    last_known_time += timedelta(seconds=rate)

                    # Adiciona os dados interpolados às listas (os últimos valores conhecidos)
            ...
            # Atualiza os dados do satélite para o próximo ciclo
            ...
            # Adiciona os dados originais (sem interpolação) às listas
            all_data.append(date)
            all_time.append(time)
            all_sat.append(sat)
            all_longitude.append(lon_ip)
        ...
        # Depois de processar todos os satélites:
        # Definir o horário final como 23:59:45
        final_hour = datetime.strptime(f"{date} 23:59:45", "%d-%m-%Y %H:%M:%S")

        # Iterar sobre cada satélite para extrapolar os dados até o final do dia
        for satellite_name, data in last_data_by_satellite.items():
            last_record = data  # Último registro do satélite
            while last_record['date_time'] < final_hour:
                last_record['date_time'] += timedelta(seconds=rate)  # Incrementa em 15 segundos

                # Adiciona os dados extrapolados (os últimos valores conhecidos) às listas
        ...
        # Após sair do loop, converta as listas para arrays NumPy (se necessário)
        ...
        # Criar um novo DataFrame combinado com os dados originais, interpolados e extrapolados
        combined_df = pd.DataFrame({
            "Date": all_date,
            "Time": all_time,
            "SAT": all_sat,
